...
# ============================ CONFIGURATION ============================
# Where the addon is installed. By default this is the project root
# (the parent of this script's folder). Override it to install into a
# WoW AddOns folder instead, for example:
# ADDONS_PATH = r"C:\Program Files (x86)\World of Warcraft\_retail_\Interface\AddOns"
ADDONS_PATH = str(Path(__file__).resolve().parent.parent)
...
def main() -> int:
    ...
    if not addons_path_text:
        print(
            "Error: ADDONS_PATH is empty.",
            file=sys.stderr
        )
        return 2
    ...
    # Record the build id so the publish step can tag the release.
